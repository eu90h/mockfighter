...
(define game-master%
  (class object% (super-new)
    ...
    (define (handle-executions api-key instance response player-incoming?)
      (define executions (instance-executions-socket instance))
      (unless (equal? #f executions)
       ; (cond [(list? response)
               (let ([users-orders (filter (lambda (o) (is-players-order? instance o)) response)])
                 (unless (null? users-orders)
                   (define users-order (car users-orders))
                   (for ([order (in-list response)])
                     (unless (equal? (order-account order) (order-account users-order))
                   (define latest-fill (newest-fill (order-fills order)))
                   (define venue-name (instance-venue-name (hash-ref instances api-key)))
                   (define symbol (instance-symbol (hash-ref instances api-key)))
                   (define accnt (order-account users-order))
                   (define-values (standing-id incoming-id standing-complete? incoming-complete?)
                     (if player-incoming?
                         (values (order-id order) (order-id users-order) (zero? (order-qty order)) (zero? (order-qty users-order)))
                         (values (order-id users-order) (order-id order) (zero? (order-qty users-order)) (zero? (order-qty order)))))
                   (send-execution api-key instance (make-hash (list (cons `ok #t)
                                                                     (cons `account accnt)
                                                                     (cons `symbol symbol)
                                                                     (cons `venue venue-name)
                                                                     (cons `price (fill-price latest-fill))
                                                                     (cons `filledAt (fill-time latest-fill))
                                                                     (cons `filled (fill-qty latest-fill))
                                                                     (cons `order order)
                                                                     (cons `standingComplete standing-complete?)
                                                                     (cons `incomingComplete incoming-complete?)
                                                                     (cons `standingID standing-id)
                                                                     (cons `incomingID incoming-id))))))))))
                        ...
                        (let ([venue-name (hash-ref order `venue #f)])
                          (if (or (equal? #f venue-name) (not (equal? (send (instance-venue (hash-ref instances api-key)) get-name) venue-name)))
                              ...
                              (let ([response (send (instance-venue instance) handle-order order)]
                                ...
                                (if (list? response)
                                    (let ([r (filter (lambda (o) (equal? account (order-account o))) response)])
                                      (handle-executions api-key instance response (equal? account (instance-owner instance)))
                                      (car r))
