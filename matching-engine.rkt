...
(define matching-engine%
  (class object% (super-new)
    ...
    (define/public (handle-order o)
      ...
      (cond [(equal? "buy" (order-direction o)) (let ([resp (handle-bid o)]) (if (hash? resp) resp o))]
    ...
    (define/public (handle-bid b)
      ...
      (cond [(= 0 (heap-count (orderbook-asks orderbook))) (if (equal? "limit" (order-type b))
                                                               (begin (orderbook-add-bid! orderbook b) b)
                                                               (begin (hash-set! b `open #f) b))]
            [(equal? "market" (order-type b)) (cross-bid b)]
            [(<= (order-price (orderbook-get-best-ask orderbook)) (order-price b))
             (cross-bid b)]
            [else (orderbook-add-bid! orderbook b) b]))
    (define/public (handle-ask a)
      (hash-set! orders (order-id a) a)
      (define b (orderbook-get-best-bid orderbook))
      (cond [(= 0 (heap-count (orderbook-bids orderbook))) (if (equal? "limit" (order-type a))
                                                               (begin (orderbook-add-ask! orderbook a) a)
                                                               (begin (hash-set! a `open #f) a))]
            [(equal? "market" (order-type a)) (cross-bid a)]
            [(and (not (equal? #f b)) (<= (order-price a) (order-price b)))
             (cross-ask a)]
            [else (orderbook-add-ask! orderbook a) a]))
    (define/public (cross-ask a)
